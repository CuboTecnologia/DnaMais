...
function onlyFloatNumbers(evento, campo) {

    var theEvent = evento || window.event;
    var key = theEvent.keyCode || theEvent.which;
    key = String.fromCharCode(key);
    var value = campo.value + key;
    var regex = /^\d+(,\d{0,2})?$/;
...
function maskCNPJ(campo, evento) {
    ...
    if (valor.length == 14 && valor.match(/^\d+$/)) {
        //44.352.357/0001-04
        var slice = valor.slice(0, 2) + ".";
        slice += valor.slice(2, 5) + ".";
        slice += valor.slice(5, 8) + "/";
        slice += valor.slice(8, 12) + "-";
        slice += valor.slice(12, 15);
